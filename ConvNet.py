...
class ConvNet:
        ...
        for layer in tqdm(range(self.net_config.total_layers), desc="Layer"):  # construct layers
            ...
            if built_for_training:
                ...
                self.best_bias[layer] = tf.Variable(
                    tf.ones([out_channels], tf.float32), dtype=tf.float32)
                self.assign_best_conv_filter[layer] = self.best_conv_filter[layer].assign(
                    self.conv_filter[layer])
                self.assign_best_bias[layer] = self.best_bias[layer].assign(
                    self.bias[layer])
            else:
                ...
                self.bias[layer] = tf.Variable(tf.random_normal(
                    [out_channels], 0, 1, tf.float32), dtype=tf.float32, name=self.bias_name[layer])

            if layer == self.net_config.total_layers - 1:
                layer_output[layer] = tf.nn.conv2d(x_input, self.conv_filter[layer], [
                                                   1, 1, 1, 1], 'SAME') + self.bias[layer]
            else:
                # Activation Function
                layer_output[layer] = tf.nn.relu(tf.nn.conv2d(
                    x_input, self.conv_filter[layer], [1, 1, 1, 1], 'SAME') + self.bias[layer])
    ...
    def restore_network_with_model_id(self, sess_in, restore_layers_in, model_id):
        ...
        if restore_layers_in > 0:
            for layer in range(restore_layers_in):
                save_dict[self.conv_filter_name[layer]
                          ] = self.conv_filter[layer]
                save_dict[self.bias_name[layer]] = self.bias[layer]
            model_id_str = np.array2string(model_id, separator='_', formatter={
                                           'int': lambda d: "%d" % d})
            model_id_str = model_id_str[1:(len(model_id_str)-1)]
            model_folder = format("%snetid%d_model%s" % (
                self.net_config.model_folder, self.net_id, model_id_str))
            ...
            print(f"Restore the first {restore_layers_in} layers.")
    ...
    def test_network_online(self, dataio, x_in, y_label, orig_loss, loss_after_training, calc_org_loss, sess_in):
        ...
        while remain_samples > 0:
            ...
            batch_xs, batch_ys = dataio.load_batch_for_test(
                load_batch_size)  # features, labels
            if calc_org_loss:
                ...
                loss_after_training_value = sess_in.run(loss_after_training, feed_dict={
    ...
    def calc_normality_test(self, residual_noise, batch_size, batch_size_for_norm_test):
        ...
        residual_noise = tf.reshape(residual_noise, [
            groups, self.net_config.label_length * batch_size_for_norm_test])
        ...
        norm_test = tf.reduce_mean(
            tf.square(skewness) + 0.25 * tf.square(kurtosis-3))
    ...
    def train_network(self, model_id):
        ...
        y_label = tf.placeholder(
            tf.float32, [None, self.net_config.label_length])

        # normality test
        if self.train_config.normality_test_enabled:
            training_loss = self.calc_normality_test(
                y_label - y_out, self.train_config.training_minibatch_size, 1)
            orig_loss_for_test = self.calc_normality_test(
                y_label - x_in, self.train_config.test_minibatch_size, 1)
            test_loss = self.calc_normality_test(
                y_label - y_out, self.train_config.test_minibatch_size, 1)
            if self.train_config.normality_lambda != np.inf:
                training_loss = tf.reduce_mean(tf.square(
                    y_out - y_label)) + training_loss * self.train_config.normality_lambda
                orig_loss_for_test = tf.reduce_mean(tf.square(
                    y_label - x_in)) + orig_loss_for_test * self.train_config.normality_lambda
                test_loss = tf.reduce_mean(
                    tf.square(y_label - y_out)) + test_loss * self.train_config.normality_lambda
        else:
        ...
        print("BEFORE TRAIN STEP", flush=True)
        # SGD_Adam
        ...
        self.restore_network_with_model_id(
            sess, self.net_config.restore_layers, model_id)
        ...
        print(f'Final minimum loss: {min_loss}' % min_loss)
        print(f'Used time for training: {(end-start).seconds}s')
        ...
        if self.res_noise_power_dict.__len__() == 0:
            ...
            if np.size(shape_data) == 1:
                ...
                for i in range(SNR_num):
                    self.res_noise_power_dict[data[i, 0]
                                              ] = data[i, 1:shape_data[1]]
    ...
    def get_res_noise_pdf(self, model_id):
        if self.res_noise_pdf_dict.__len__() == 0:
            ...
            if np.size(shape_data) == 1:
                ...
                SNR_num = shape_data[0]
                for i in range(SNR_num):
                    self.res_noise_pdf_dict[data[i, 0]
                                            ] = data[i, 1:shape_data[1]]
