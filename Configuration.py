...
class TopConfig:
    ...
    def __init__(self):
        ...
        self.feature_map_nums = np.array(
            [64, 32, 16, 1])  # The last element must be 1
    ...
    def parse_cmd_line(self, argv):
        ...
        while id < len(argv):
            ...
            # Simulation
                ...
                print(
                    'Corr para for simulation is set to {self.corr_para_simu:.2f}')
            elif argv[id] == '-SameModelAllNets':
                self.same_model_all_nets = argv[id+1] == 'True'
                print(
                    f'same_model_all_nets is set to {self.same_model_all_nets}')
            # BP decoding
            elif argv[id] == '-BP_IterForGenData':
                self.BP_iter_nums_gen_data = np.fromstring(
                    argv[id+1], np.int32, sep=' ')
                print(
                    f'BP iter for gen data is set to: {np.array2string(self.BP_iter_nums_gen_data)}')
            elif argv[id] == '-BP_IterForSimu':
                self.BP_iter_nums_simu = np.fromstring(
                    argv[id+1], np.int32, sep=' ')
                print(
                    f'BP iter for simulation is set to: { np.array2string(self.BP_iter_nums_simu)}')
            # CNN config
                ...
                self.filter_sizes = np.fromstring(
                    argv[id+1], np.int32, sep=' ')
                print(
                    f'Filter sizes are set to {np.array2string(self.filter_sizes)}')
            elif argv[id] == '-FeatureMap':
                self.feature_map_nums = np.fromstring(
                    argv[id + 1], np.int32, sep=' ')
                print(
                    f'Feature map numbers are set to {np.array2string(self.feature_map_nums)}')
            # training
                ...
                self.SNR_set_gen_data = np.fromstring(
                    argv[id+1], dtype=np.float32, sep=' ')
                print(
                    f'SNR set for generating data is set to {np.array2string(self.SNR_set_gen_data)}.')
            else:
...
class NetConfig:
    def __init__(self, top_config):
        ...
        # the length of this array must be the same with the number of cnn layers
        self.node_num_each_layer = np.ones(
            top_config.layer_num, dtype=np.int32) * self.feature_length
