...
class TrainingDataIO:
    ...
    def load_next_mini_batch(self, mini_batch_size, factor_of_start_pos=1):
        ...
        if mini_batch_size > self.total_trainig_samples:
            print("Mini batch size should not be larger than total sample size!", flush=True)
        self.fin_feature.seek(
            (self.feature_length * 4) * (sample_id // factor_of_start_pos * factor_of_start_pos), 0
