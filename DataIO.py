...
class TrainingDataIO:
    ...
    def load_next_mini_batch(self, mini_batch_size, factor_of_start_pos=1):
        '''
        loadS the next batch where the datas in the batch are from a continuous memory block
        ...
        self.fin_label.seek((self.label_length * 4) *
                            (sample_id//factor_of_start_pos*factor_of_start_pos), 0)

        while True:
            new_feature = np.fromfile(
                self.fin_feature, np.float32, self.feature_length * remain_samples)
            new_label = np.fromfile(
                self.fin_label, np.float32, self.label_length * remain_samples)
...
class TestDataIO:
    ...
    def load_batch_for_test(self, batch_size):
        ...
        if np.size(self.all_features) == 0:
            ...
            self.all_labels = np.fromfile(
                self.fin_label, np.float32, self.label_length * self.test_sample_num)
            self.all_features = np.reshape(
                self.all_features, [self.test_sample_num, self.feature_length])
            self.all_labels = np.reshape(
                self.all_labels, [self.test_sample_num, self.label_length])

        features = self.all_features[self.data_position:(
            self.data_position + batch_size), :]
        labels = self.all_labels[self.data_position:(
            self.data_position + batch_size), :]
        self.data_position += batch_size
...
class NoiseIO:
    def __init__(self, blk_len, read_from_file, noise_file, cov_1_2_mat_file_gen_noise, rng_seed=None):
        ...
        if read_from_file:
            ...
            cov_1_2_mat = np.fromfile(
                fin_cov_file, np.float32, blk_len*blk_len)
            ...
            self.awgn_noise = tf.placeholder(
                dtype=tf.float32, shape=[None, blk_len])
    ...
    def reset_noise_generator(self):
        '''
        Resets the file pointer or the rng generator to generate the same noise data
        '''
        if self.read_from_file:
    ...
    def generate_noise(self, batch_size):
        if self.read_from_file:
            noise = np.fromfile(self.fin_noise, np.float32,
                                batch_size * self.blk_len)
            ...
            noise = self.sess.run(self.noise_tf, feed_dict={
                                  self.awgn_noise: noise_awgn})
