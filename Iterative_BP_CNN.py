...
def stat_prob(x, prob):
    ...
    [hist, _] = np.histogram(x, np.int32(
        np.round(2*(-min_v) / qstep)), [min_v, -min_v])
    if np.size(prob) == 0:
        prob = hist
    else:
        prob = prob + hist
...
def simulation_colored_noise(linear_code, top_config, net_config, simutimes_range, target_err_bits_num, batch_size):
    ...
    noise_io = DataIO.NoiseIO(top_config.N_code, False,
                              None, top_config.cov_1_2_file_simu, rng_seed=0)
    ...
    max_batches, residual_times = np.array(
        divmod(max_simutimes, batch_size), np.int32)
    if residual_times != 0:
        max_batches += 1

    # generate out ber file
    bp_str = np.array2string(bp_iter_num, separator='_', formatter={
                             'int': lambda d: "%d" % d})
    bp_str = bp_str[1:(len(bp_str) - 1)]
    ber_file = format('%sBER(%d_%d)_BP(%s)' %
                      (net_config.model_folder, N, K, bp_str))
    ...
    for SNR in tqdm(SNRset, desc="SNR"):
        ...
        for ik in tqdm(range(0, max_batches), desc="Batch"):
            print('Batch %d in total %d batches.' %
                  (ik, int(max_batches)), end=' ')
                ...
                if iter < denoising_net_num:
                    if top_config.update_llr_with_epdf:
                        ...
                        res_noise_power = conv_net[iter].get_res_noise_power(
                            model_id, SNRset).get(np.float32(SNR))
...
def generate_noise_samples(linear_code, top_config, net_config, train_config, bp_iter_num, net_id_data_for, generate_data_for, noise_io, model_id):
    ...
    if generate_data_for == 'Training':
        batch_size_each_SNR = int(
            train_config.training_minibatch_size // np.size(train_config.SNR_set_gen_data))
        total_batches = int(train_config.training_sample_num //
                            train_config.training_minibatch_size)
    elif generate_data_for == 'Test':
        batch_size_each_SNR = int(
            train_config.test_minibatch_size // np.size(train_config.SNR_set_gen_data))
    ...
    for ik in tqdm(range(0, total_batches), desc="Generating data batch"):
...
def analyze_residual_noise(linear_code, top_config, net_config, simutimes, batch_size):
    ...
    max_batches, residual_times = np.array(
        divmod(simutimes, batch_size), np.int32)
        ...
        for ik in tqdm(range(0, max_batches), desc="Batch"):
            ...
            for iter in tqdm(range(0, net_id_tested+1), desc="Net ID tested"):
                ...
                u_BP_decoded = bp_decoder.decode(
                    LLR.astype(np.float32), bp_iter_num[iter])
                noise_before_cnn = y_receive - (u_BP_decoded * (-2) + 1)
                noise_after_cnn = sess.run(denoise_net_out[iter], feed_dict={
                                           denoise_net_in[iter]: noise_before_cnn})
                s_mod_plus_res_noise = y_receive - noise_after_cnn
