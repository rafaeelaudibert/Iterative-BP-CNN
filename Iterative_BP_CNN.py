...
def stat_prob(x, prob):
    ...
    prob = hist if np.size(prob) == 0 else prob + hist
...
def simulation_colored_noise(linear_code, top_config, net_config, simutimes_range, target_err_bits_num, batch_size):
    ...
    if top_config.corr_para != top_config.corr_para_simu:
        ber_file = format(f"{ber_file}_SimuCorrPara{top_config.corr_para_simu:.2f}")
    if top_config.same_model_all_nets:
        ber_file = format(f"{ber_file}_SameModelAllNets")
    if top_config.update_llr_with_epdf:
        ber_file = format(f"{ber_file}_llrepdf")
    if denoising_net_num > 0:
        ...
        ber_file = format("{ber_file}_model{model_id_str}")
    if np.size(SNRset) == 1:
        ber_file = format("{ber_file}_{SNRset[0]:.1f}dB")

    ber_file = format(f"{ber_file}.txt")
...
def analyze_residual_noise(linear_code, top_config, net_config, simutimes, batch_size):
    ...
    loss_file_name = format(
        f"{net_config.residual_noise_property_folder}residual_noise_property_netid{int(net_id_tested)}_model{model_id_str}.txt"
    )
    ...
    for SNR in tqdm(SNRset, desc="SNR"):
        ...
        if top_config.update_llr_with_epdf:
            ...
            fout_loss.write(f"{str(SNR)}\t{str(loss)}\n")

    fout_loss.close()
    end = datetime.datetime.now()
    print(f"Time: {int((end - start).seconds)}s")
    print("end\n", flush=True)
    sess.close()
