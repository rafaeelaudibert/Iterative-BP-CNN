...
class GetMatrixForBPNet:
    '''
    Calculates the matrices used to perform BP process with matrix operation
    '''
    ...
    def __init__(self, test_H, loc_nzero_row):
        ...
        self.loc_nzero1 = self.loc_nzero_row[1,
                                             :] * self.n + self.loc_nzero_row[0, :]
        self.loc_nzero2 = np.sort(self.loc_nzero1)
        self.loc_nzero_line = np.append([np.mod(self.loc_nzero2, self.n)], [
                                        self.loc_nzero2 // self.n], axis=0)
        self.loc_nzero4 = self.loc_nzero_line[0,
                                              :] * self.n + self.loc_nzero_line[1, :]
    ...
    def get_Matrix_VC(self):
        ...
        H_xe_last_to_y = np.zeros(
            [self.n, self.num_all_edges], dtype=np.float32)
        Map_row_to_line = np.zeros([self.num_all_edges, 1])

        for i in tqdm(range(0, self.num_all_edges), desc="Row to line"):
            Map_row_to_line[i] = np.where(
                self.loc_nzero1 == self.loc_nzero2[i])

        map_H_row_to_line = np.zeros(
            [self.num_all_edges, self.num_all_edges], dtype=np.float32)

        for i in tqdm(range(0, self.num_all_edges), desc="H Row to line"):
            map_H_row_to_line[i, int(Map_row_to_line[i])] = 1

        count = 0
        for i in tqdm(range(0, self.n), desc="n"):
    ...
    def get_Matrix_CV(self):
        ...
        for i in tqdm(range(0, self.num_all_edges), desc="Line to row"):
            Map_line_to_row[i] = np.where(
                self.loc_nzero4 == self.loc_nzero5[i])

        map_H_line_to_row = np.zeros(
            [self.num_all_edges, self.num_all_edges], dtype=np.float32)

        for i in tqdm(range(0, np.size(self.loc_nzero1)), desc="H line to row"):
            map_H_line_to_row[i, int(Map_line_to_row[i])] = 1

        count = 0
        for i in tqdm(range(0, self.m), desc="m"):
...
class BP_NetDecoder:
    ...
    def one_bp_iteration(self, xe_v2c_pre_iter, H_sumC_to_V, H_sumV_to_C, xe_0):
        ...
        xe_sum_log_img = tf.matmul(H_sumC_to_V, tf.multiply(
            tf.truediv((1 - xe_tanh_temp), [2.0]), [3.1415926]))
    ...
    def build_network(self):
        '''
        Builds the network for one BP iteration
        '''
        ...
        bp_out_llr = tf.add(llr_into_bp_net, tf.matmul(
            self.H_xe_v_sumc_to_y, xe_v_sumc))
        dec_out = tf.transpose(tf.floordiv(
            1-tf.to_int32(tf.sign(bp_out_llr)), 2))
    ...
    def decode(self, llr_in, bp_iter_num):
        ...
        if real_batch_size != self.batch_size:  # padding zeros
            # re-create an array and will not influence the value in
            llr_in = np.append(llr_in, np.zeros(
                [self.batch_size-real_batch_size, num_v_node], dtype=np.float32), 0)
            # original llr array.
        self.sess.run(self.llr_assign, feed_dict={
                      self.llr_placeholder: llr_in})
